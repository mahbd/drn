...
public class ChatQueryService {
    ...
    private  String getChatResponse(String query) {
        String chatQueryUrl = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro-latest:generateContent?key=" + System.getenv("GEMINI_API_KEY");
        PartItem partItemRequest = new PartItem(query);
        ContentItem contentItemRequest = new ContentItem(List.of(partItemRequest));
        GeminiRequest request = new GeminiRequest(List.of(contentItemRequest));
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<GeminiRequest> entity = new HttpEntity<>(request, headers);

        ResponseEntity<GeminiResponse> response = restTemplate.exchange(chatQueryUrl, HttpMethod.POST, entity, GeminiResponse.class);
        if (!response.getStatusCode().is2xxSuccessful()) {
            return "Sorry, I'm not able to answer your question at the moment. Please try again later. Status: " + response.getStatusCode();
        }
        ...
        geminiResponse.getCandidates().forEach(candidateItem -> candidateItem.getContent().getParts().forEach(partItem -> responseBuilder.append(partItem.getText())));
